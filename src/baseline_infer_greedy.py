...
def main():
    ...
    ap.add_argument("--out_file",  default=os.path.join("logs","baseline_greedy_newtok.jsonl"))
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model_name, dtype=torch.bfloat16 if torch.cuda.is_available() else torch.float32).to("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    ...
    with open(args.out_file, "w", encoding="utf-8") as fout:
        for ex in tqdm(ds):
            ...
            inputs = tok(prompt, return_tensors="pt", truncation=True, max_length=args.max_prompt_len).to(model.device)
            with torch.no_grad():
